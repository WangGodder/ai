...
// 表示图表位置
const legendPositions = ["top", "right", "bottom", "left", "hide"];
// 支持的图表类型
const chartTypes = ["line", "bar", "radar", "pie", "doughnut"];
// 颜色列表
const colorsArray = ["red", "rebeccapurple", "lime", "green", "blue", "yellow", "aqua", "pink", "gold", "gray", "black", "white"];
 ...
 */
function baseDataToChartConfig(baseData, type) {
    let data, options;
 ...
 * @param value
 */
function changeLegendPosition(chart, position) {
    if (position == 'hide') {
...
function changeDatasetBorderColor(chart, index, color) {
...
function getRandomBaseData(datasetNum, dataNum) {
