...
(function (win) {
    ...
    var html = '<div class="page_list clear">\
        {{if pageNo!=1}}\
        ...
        <span class="ml10">共{{ totalPage }}页</span>\
    ...
    win.page = function(selector, pageInfo){
        var $page = $(pageRender(pageInfo));
        $page.find('.pageSize').val(pageInfo.pageSize);

        $(selector).html($page);

        var fn = pageInfo.callback;
        if (fn) {
            $page.find('a[page]:not(.current)').click(function(){
                fn($(this).attr('page'), $(this).closest('div').find('.pageSize').val());
                $("html,body").animate({scrollTop: 470});
            });

            $page.find('.pageSize').change(function(){
                fn(1, $(this).val());
                $("html,body").animate({scrollTop: 470});
            });
        }
